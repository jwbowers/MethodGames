...
## To reproduce socmeth.Rout, run this file in batch mode: R CMD BATCH socmeth.R
...
makedatamatrix<-function(nfeatures,N){
  ...
  ## but to have half 1 and half 0 on average.
...
makeoutcome<-function(X,thetruth){
  ## X: a data.table object such as that arising from makedatamatrix()
  ...
  ### notice data.table syntax where the second element is an thetruthession of
  ### column names
  as.numeric(X[,eval(parse(text=thetruth))] )
}

makemodelmatrix<-function(thedata,interactionorder){
  ## thedata: a data.table or data.frame object
  ## interactionorder: a scalar integer for the number of interactions to search over.
  ### Notice: No intercept
...
fitfn<-function(y,X,DAT, thetruth=thetruth){
  ...
  ## Adaptive Lasso (L1-penalized logistic regression weighted by an L2-penalized logistic regression)
  ### Choose lambda by minimizing misclassification in k-fold cross-validation
  ...
  ## QCA
  ### Use both 0 and 1 outcome as to be explained.
  ### Code "found truth" if either finds the truth.
...
myfn.maker<-function(nfeatures,N,thetruth,interactionorder){
  ...
  ## thetruth: is a character evaluating to a logical thetruthession of the column names of X
  ## interactionorder: a scalar integer for the number of interactions to search over.
  force(nfeatures);force(N);force(thetruth);force(interactionorder)
  function(){
...
## I am going through some extra work to make these results reproducible
...
apply(easygameresults.arr,1,function(x){mean(is.na(x)) }) ## what proportion NAs in each method?
...
apply(hardgameresults.arr,1,function(x){mean(is.na(x)) }) ## what proportion NAs in each method?
